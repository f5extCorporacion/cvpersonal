...
// 🎨 COLORES PASTEL
const vec3 PASTEL_PINK  = vec3(255.0, 182.0, 193.0) / 255.0;   // #FFB6C1
const vec3 PASTEL_BLUE  = vec3(173.0, 216.0, 230.0) / 255.0;   // #ADD8E6
const vec3 PASTEL_LAVENDER = vec3(230.0, 230.0, 250.0) / 255.0; // #E6E6FA
const vec3 PASTEL_MINT = vec3(189.0, 252.0, 201.0) / 255.0;    // #BDFCC9
const vec3 PASTEL_PEACH = vec3(255.0, 218.0, 185.0) / 255.0;   // #FFDAB9
const vec3 PASTEL_YELLOW = vec3(255.0, 255.0, 224.0) / 255.0;  // #FFFFE0
const vec3 PASTEL_PURPLE = vec3(204.0, 204.0, 255.0) / 255.0;  // #CCCCFF
...
vec3 background_color(vec2 uv) {
  ...
  // Mezcla de colores pastel suaves
  col += mix(PASTEL_BLUE, WHITE, smoothstep(0.0, 1.0, abs(m)));
  col += mix(PASTEL_PINK, WHITE, smoothstep(0.0, 1.0, abs(m - 0.8)));
  col += mix(PASTEL_LAVENDER, WHITE, smoothstep(0.0, 1.0, abs(m + 0.5)));
  
  return col * 0.3; // Más suave y brillante
}
...
vec3 getLineColor(float t, vec3 baseColor) {
  ...
  return gradientColor * 0.6; // Más suave para efecto pastel
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Añadir un ligero brillo pastel al fondo
...
<template>
  <div class="relative min-h-screen w-full overflow-hidden bg-gradient-to-br from-pink-50 via-blue-50 to-purple-50">
    <!-- FloatingLines como fondo -->
    <div 
      ref="containerRef" 
